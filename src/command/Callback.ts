...
import { playerRuleFormInstance } from "../otherForm/playerRule.js";
...
const call: {
    ...
    leveldb: (_: Command, ori: CommandOrigin, out: CommandOutput, result: commandResult) => {
        ...
        switch (result.leveldb) {
            case "import":
                leveldb.importDataType(result.isOldData) ? logger.info(`导入成功！`) : logger.error(`导入失败！`);
                break;
            case "export":
...
export function commandCallback(_: Command, ori: CommandOrigin, out: CommandOutput, result: commandResult) {
    ...
    switch (result.action) {
        ...
        case "pr":
            ...
            break;
        case "rule":
            ori.player ? playerRuleFormInstance.index(ori.player) : sendPlayersUse(out);
            break;
        case "back":
