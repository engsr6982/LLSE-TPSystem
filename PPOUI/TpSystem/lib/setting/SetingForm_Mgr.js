...
import { FileOperation, Gm_Tell, db } from "../modules/cache.js";
 ...
 * @returns 
 */
export function SetingForm_Mgr(pl) {
    if (!Perm.isOP(pl.xuid)) return pl.tell(Gm_Tell + "无权限!");
    ...
    pl.sendForm(fm, (pl, id) => {
        switch (id) {
            ...
            case 3: FileOperation.readFile(); pl.tell(Gm_Tell + '操作完成！'); break;
...
class HomePanel_Mgr {
    ...
    static HomePanel(pl) {
        ...
        pl.sendForm(fm, (pl, id) => {
            ...
            if (id == AllPlayerData.length) return SetingForm_Mgr(pl);
...
class WarpPanel_Mgr {
    static WarpPanel(pl) {
        ...
        pl.sendForm(fm, (pl, id) => {
            if (id == null) return Other.CloseTell(pl);
            if (id == 0) return SetingForm_Mgr(pl);
            if (id == 1) {
...
class MergeRequest_Mgr {
    static MergeRequest_Panel(pl) {
        ...
        pl.sendForm(fm, (pl, id) => {
            if (id == null) return Other.CloseTell(pl);
            if (id == MergeRequest.length) return SetingForm_Mgr(pl);
