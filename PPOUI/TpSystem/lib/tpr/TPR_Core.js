...
function CheckWorld(player) {
    const { MainWorld, Infernal, Terminus } = Config.TPR;
    if (player.pos.dimid === 0 && MainWorld === false) return false;
    if (player.pos.dimid === 1 && Infernal === false) return false;
    if (player.pos.dimid === 2 && Terminus === false) return false;
    return true;
}
 ...
 */
export function TPR_Core(player) {
    let Interval_ID = null;
    let BackUpPos = null;
    try {
        if (!CheckWorld(player)) {
            player.tell(`${Gm_Tell}随机传送在当前维度不可用！`);
            return;
        }

        if (!Money_Mod.DeductEconomy(player, Config.TPR.Money)) return;

        // 开始准备传送所需参数
        player.tell(`${Gm_Tell}准备传送...`);
        ...
        if (player.pos.dimid === 1) { // 如果玩家在地狱维度
            forPosInfo = {
                startPos: 100,
                forEnd: 0,
                failureEnd: 10
        ...
        function _run() {
            try {
                ...
                for (Pos_Y; Pos_Y > forPosInfo.forEnd; Pos_Y--) {
                    ...
                    } else if (Pos_Y <= forPosInfo.failureEnd || ["minecraft:lava", "minecraft:flowing_lava"].indexOf(block_Obj.type) !== -1) {
        ...
        // 传送函数
        function Transmission(YY) {
            player.teleport(new FloatPos(InitialTargetCoordinates.x + 0.5, YY + 2, InitialTargetCoordinates.z + 0.5, InitialTargetCoordinates.dimid));
            player.tell(`${Gm_Tell}传送完成！`);
    ...
    function _fail(player, e) {
        ...
        if (player.pos != null) {
            ...
            player.tell(`${Gm_Tell}§c插件遇到未知错误, 传送失败！`);
            Money_Mod.addMoney(player, Config.TPR.Money);
        } else {
