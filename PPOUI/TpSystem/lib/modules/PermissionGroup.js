...
export default class PermissionGroup {
    /**
     * 实例化Permission_Group
     * @param {String} path tps 权限组保存路径 ./plugins/Perm/data.json
     */
    constructor(path) {
        this._path = path;
        this._data = new JsonConfigFile(this._path, JSON.stringify({ op: [], user: {} }));
    }
     ...
     */
    hasUserPerm(xuid, perm) {
        const tmp = this.getUserInGroup(xuid);
...
/**实例化导出 权限组 */
export const Perm = new PermissionGroup(_filePath + "Perm.json");